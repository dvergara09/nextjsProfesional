...
const Pagination = ({ totalItems, itemsPerPage, nums, setOffset }) => {
  const items = [];
  const [current, setCurrent] = useState(1);
  const totalPage = Math.ceil(totalItems / itemsPerPage);
  const end = Math.min(Math.max(nums * 2 + 2, nums + current + 1), totalPage + 1);
  const start = Math.min(Math.max(end - (nums * 2 + 1), 1), Math.max(current - nums, 1));
